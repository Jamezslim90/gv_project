...
class Payment(models.Model):
    PAYMENT_METHOD = (
        ('Monnify', 'Monnify'),
        ('Paystack', 'Paystack'), # Only for Indian Students.
    )
...
class Order(models.Model):
    ...
    def order_placed_to(self):
        return ", ".join([str(i) for i in self.doctors.all()])

    def get_total_by_doctor(self):
        doctor = Doctor.objects.get(user=request_object.user)
        subtotal = 0
        tax = 0
        tax_dict = {}
        if self.total_data:
            total_data = json.loads(self.total_data)
            data = total_data.get(str(doctor.id))
...
class OrderedItem(models.Model):
    ...
    payment = models.ForeignKey(Payment, on_delete=models.SET_NULL, blank=True, null=True)
    user = models.ForeignKey(User, on_delete=models.CASCADE)
    item = models.ForeignKey(ConsultationItem, on_delete=models.CASCADE)
